...
XmlSerializer serializer = new(typeof(List<FoodDisplay>), new XmlRootAttribute("Food_Display_Table"));
List<FoodDisplay> displays = serializer.Deserialize(reader) as List<FoodDisplay>;

Console.WriteLine($"Count: {displays.Count}");
Console.WriteLine($"First item: {displays.First()}");


[XmlType("Food_Display_Row")]
public record FoodDisplay 
{
